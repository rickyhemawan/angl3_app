...
class AnimatedProgressBar extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double getFixedAnimationValue() {
      if (animation.value < 0.4) return 0.4;
    ...
    if (widget.displayText != null) {
      Widget textProgress = new Container(
        ...
        child: Container(
          ...
          child: Text(
            (animation.value * widget.maxValue).toStringAsFixed(1) +
                widget.displayText,
            softWrap: false,
            overflow: TextOverflow.ellipsis,
            style: Theme.of(context).textTheme.headline6.copyWith(
                  color: widget.progressColor.computeLuminance() > 0.5
                      ? Colors.black
                      : Colors.white,
                ),
          ),
    ...
    return Directionality(
      ...
      child: Container(
        ...
        child: Flex(
          ...
          children: <Widget>[
            Expanded(
                flex: (getFixedAnimationValue() * 100).toInt(),
                child: Stack(children: progressWidgets)),
            Expanded(
              flex: 100 - (getFixedAnimationValue() * 100).toInt(),
              child: Container(),
            )
          ],
        ),
